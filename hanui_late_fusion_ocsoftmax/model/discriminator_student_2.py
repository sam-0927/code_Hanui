...
class MRD(nn.Module):
    def __init__(
        ...
        #self.conv_post = WNConv1d(ch, 2, 3, 1, padding=1)
    ...
    def forward_logit(self, x, x_recon):
        x_mag, _ = self.spectrogram_mag(x)  # [5,32,1,13,102] = [num_band, B, 1, num_frame, band_freq_range_0]
        x_mag_recon, _ = self.spectrogram_mag(x_recon)
        ...
        x_mag = x_mag - x_mag_recon
        x_bands = [x_mag[..., b[0] : b[1]] for b in self.bands]
...
class Discriminator_student_2(ml.BaseModel):
    ...
    def forward_logit_combine(self, x, x_recon): # [32, 1, 6400]
        ...
        x_recon = self.preprocess(x_recon)
        logits = [d.forward_logit(x, x_recon) for d in self.discriminators]
        # logits_log = [d.forward_logit_log(x, x_recon) for d in self.discriminators[2:]]
