...
class Train(object):
    def __init__(self, rank, a):
        ...
        self.scheduler_d = torch.optim.lr_scheduler.ExponentialLR(self.optim_d, 0.97)
    ...
    def _run_one_epoch(self, train=True):
        ...
        for i, batch in enumerate(pbar):
            ...
            # mpd_logit_list = []
            # for i in range(len(d_fake_mpd)):
            #     mpd_logit_list.append(self.discriminator.discriminators_mpd[i](d_real_mpd[i][:-1], d_fake_mpd[i][:-1]))
            # summed_logit_mpd = sum(mpd_logit_list)
            # disc_mpd_loss = self.ce_loss(summed_logit_mpd, key)
            ...
            loss_logs.append(disc_loss.item())
            pbar.set_description("CE loss: {:.5f}".format(disc_mrd_loss.item()))
            
            self.global_step += 1
        # if self.global_step > 635250:   # after best_321
    ...
    def _run_one_epoch_eval(self):
        ...
        self.writer.add_scalar(f"validation/mrd_loss", avg, self.global_step)
        self.writer.add_scalar(f"validation/EER", eer, self.global_step)
        return eer
...
def main():
    ...
    #parser.add_argument('--checkpoint_path_disc', default='/home/sam/singfake/outdir/mix_draft/chkpt')
    #parser.add_argument('--name_disc', default='best_model.ckpt-344_d.pt')

    parser.add_argument('--checkpoint_path_student', default="/home/sam/singfake/outdir/mix_all/chkpt")
    parser.add_argument('--name_student', default='model.ckpt-last_g.pt')
    parser.add_argument('--checkpoint_path_disc', default='/home/sam/singfake/outdir/mix_all/chkpt')
