...
class Train(object):
    ...
    def _run_one_epoch(self, train=True):
        ...
        # loss_names = ['disc', 'mpd_1','mpd_2','mpd_3', 'msd','mrd_1','mrd_2','mrd_3','mrd_4','mrd_5','mrd_1_log','mrd_2_log','mrd_3_log','mrd_4_log','mrd_5_log']
        ...
        for i, batch in enumerate(pbar):
            ...
            if not train:
                ...
                # pred_label_log = pred_label_log.unsqueeze(0)
                # pred_label_chroma = pred_label_chroma.unsqueeze(0)
            
            loss = self.ce_loss(pred_label, key)
            # loss_log = self.ce_loss(pred_label_log, key)
            # loss_chroma = self.ce_loss(pred_label_chroma, key)
            disc_loss = loss
            
            disc_losses = []
            disc_losses.append(loss)
            # disc_losses.append(loss_log)
            # disc_losses.append(loss_chroma)
            ...
            loss_values = {
                ...
                # "mrd_chroma": disc_losses[1],
                # "mrd_3": disc_losses[2],
                # "mrd_4": disc_losses[3],
                # "mrd_5": disc_losses[8],
                # "mrd_1_log": disc_losses[3],
                # "mrd_2_log": disc_losses[4],
                # "mrd_3_log": disc_losses[5],
                # "mrd_4_log": disc_losses[12],
                # "mrd_5_log": disc_losses[13],
            }
            for k in loss_logs:
                loss_logs[k].append(loss_values[k].item())
            
            pbar.set_description("CE loss: {:.5f} Mag: {:.5f}".format(disc_loss.item()/len(pred_label), loss.item()/len(pred_label)))
        ...
        # avg_losses = tuple(np.mean(loss_logs[k]) for k in ["disc", 'mpd_1','mpd_2','mpd_3', 'msd','mrd_1','mrd_2','mrd_3','mrd_4','mrd_5','mrd_1_log','mrd_2_log','mrd_3_log','mrd_4_log','mrd_5_log'])
