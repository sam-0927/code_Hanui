...
class MFDataset(torch.utils.data.Dataset):
    ...
    def _get_tracklist(self):
        ...
        if self.train:
            ...
            names = glob(os.path.join(path.replace('mixtures','mixtures_T03'), 'aac_64k','*_T03_*')) + glob(os.path.join(path.replace('mixtures','mixtures_T03'), 'mp3_128k', '*_T03_*'))
    ...
    def _load_track(self, index):
        ...
        return sig, key, track_name.split('/')[-1], sig_len
