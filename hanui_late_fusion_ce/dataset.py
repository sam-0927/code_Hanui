...
class MFDataset(torch.utils.data.Dataset):
    ...
    def _get_tracklist(self):
        ...
        #if self.train:
        #    names = glob(os.path.join(path, '*_Training_*'))    # recon_0_Training_1006.wav
        #elif self.valid:
        #    names = glob(os.path.join(path, '*_Validation_*'))
        if self.train and not self.spoof:
            names = glob(os.path.join(path, '*0_Training_*'))    # 0: bonafide, 1: spoof
        elif self.eval_t01:
            if self.spoof:
        ...
        elif self.valid and not self.disc:
            if self.spoof:
                names = glob(os.path.join(path, '*1_Validation_*'))
            else:
                names = glob(os.path.join(path, '*0_Validation_*'))
        elif self.eval_t04:
            if self.spoof:
                names = glob(os.path.join(path, '../mixtures_T04_only_per', '*1_T04_*'))
            else:
                names = glob(os.path.join(path, '../mixtures_T04_only_per', '*0_T04_*'))
        elif self.eval_t03:
            if self.spoof:
                names = glob(os.path.join(path, '../mixtures_T03/aac_64k/1_T03_*')) + \
                        glob(os.path.join(path, '../mixtures_T03/mp3_128k/1_T03_*'))
            else:
                names = glob(os.path.join(path, '../mixtures_T03/aac_64k/0_T03_*')) + \
                        glob(os.path.join(path, '../mixtures_T03/mp3_128k/0_T03_*'))
    ...
    def _get_chunk_indices(self, track_name):
        ...
        num_frames_to_read = num_frames_total
