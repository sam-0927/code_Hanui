...
from model.discriminator_student_both import Discriminator_student_both
...
class Train(object):
    def __init__(self, rank, a):
        ...
        self.discriminator = Discriminator_student_both().to(self.device)
    ...
    def _run_one_epoch(self, train=True):
        ...
        for i, batch in enumerate(pbar):
            sig, key, names = batch
            sig = torch.autograd.Variable(sig.to(self.device, non_blocking=False))
            ...
            pred_label = self.discriminator.forward_logit_both(sig.audio_data, recons.audio_data)
            if not train:
...
def main():
    ...
    parser.add_argument('--checkpoint_path_student', default="outdir/mix_student/chkpt")
