...
class Evaluator(object):
    ...
    def eval(self):
        ...
        with torch.no_grad():
            ...
            '''
            print('T04 EER')
            val_err_04 = self._run_one_epoch_analysis(self.test_loader_04)
            print('Validation EER')
            val_err_01 = self._run_one_epoch_analysis(self.valid_dataset)
            print('T01 EER')
            val_err_01 = self._run_one_epoch_analysis(self.test_loader_01)
            print('T02 EER')
            val_err_02 = self._run_one_epoch_analysis(self.test_loader_02)
            print('T03 EER')
            val_err_04 = self._run_one_epoch_analysis(self.test_loader_03)
            '''
    ...
    def _run_one_epoch_analysis(self, data_loader):
        pbar = tqdm(data_loader, total=len(data_loader))
        ...
        for i, batch in enumerate(pbar):
            ...
            for m in range(3):
                for b in range(5):
                    ...
                    mrd_combine_logit_list.append(self.discriminator.discriminators_mrd_band[fc_idx](emb))
                    fc_idx += 1
            logit_mrd_1 = mrd_combine_logit_list[0]
            logit_mrd_2 = mrd_combine_logit_list[1]
            logit_mrd_3 = mrd_combine_logit_list[2]

            pred_label_1 = logit_mrd_1
            pred_label_2 = logit_mrd_2
            pred_label_3 = logit_mrd_3
            ...
            for i in range(len(key)):
                if key[i] == 1: # spoof
                    target_scores_1.append(batch_score_1[i])
                    target_scores_2.append(batch_score_2[i])
                    target_scores_3.append(batch_score_3[i])
                else:
                    nontarget_scores_1.append(batch_score_1[i])
                    nontarget_scores_2.append(batch_score_2[i])
                    nontarget_scores_3.append(batch_score_3[i])
            
        eer = eer_from_scores(target_scores_1, nontarget_scores_1)
        print('mrd 1:{}'.format(eer))
        eer = eer_from_scores(target_scores_2, nontarget_scores_2)
        print('mrd 2:{}'.format(eer))
        eer = eer_from_scores(target_scores_3, nontarget_scores_3)
        print('mrd 3:{}'.format(eer))
        return eer
