...
from model.discriminator_student import Discriminator_student
...
class Train(object):
    def __init__(self, rank, a):
        ...
        self.discriminator = Discriminator_student().to(self.device)
        self.ema_model = Discriminator_student().to(self.device)   # EMA 모델 따로 생성
        for p in self.ema_model.parameters():
            p.requires_grad_(False)  # 학습되지 않도록 막음
        ...
        # self.optim_ema = EMAOptimizer(self.optim_d, self.discriminator, decay=0.999)
        ...
        self.cv_loss_ema = torch.Tensor(self.epochs)
        for epoch in range(self.epochs):
            self.tr_loss[epoch] = 10000.0
            self.cv_loss[epoch] = 10000.0
            self.cv_loss_ema[epoch] = 10000.0
        ...
        self.ce_loss = nn.CrossEntropyLoss(weight,reduction='none')
    ...
    def save_checkpoint(self, checkpoint_path, epoch, save_trainer=True):
        ...
        torch.save({
                        ...
                        "cv_loss_ema": self.cv_loss_ema,
                        "discriminator_ema": self.ema_model.state_dict(),
                        }, d_chkpt_path)
        if logging:
    ...
    def save_checkpoint_ema(self, checkpoint_path, epoch, save_trainer=True):
        model_chkpt_path = os.path.join(self.chkpt_dir, checkpoint_path)
        d_chkpt_path = model_chkpt_path.split('_ema.pt')[0] + '_d.pt'
        torch.save({
                        "cv_loss_ema": self.cv_loss_ema,
                        "discriminator_ema": self.ema_model.state_dict(),
                        }, d_chkpt_path)
    ...
    def _reset(self):
        ...
        self.best_val_loss_ema = float("inf")
        ...
        if os.path.exists(last_chkpt_d):
            ...
            self.cv_loss_ema[:self.start_epoch] = checkpoint["cv_loss_ema"][:self.start_epoch]
            self.best_val_loss = min(self.cv_loss[:self.start_epoch])
            self.best_val_loss_ema = min(self.cv_loss_ema[:self.start_epoch])
    ...
    def train(self):
        for epoch in range(self.start_epoch, self.epochs):
            ...
            with torch.no_grad():
                ...
                ce_loss, best_mrd_loss_ema = self._run_one_epoch(train=False, use_ema=True)
                val_loss_ema = ce_loss
                
            with open(f'{self.log_dir}/val_curve.log', 'a') as fp:
                msg = 'Epoch: {}\t validation loss: {:.5f}\t\n'.format(epoch+1, val_loss)
                fp.write(msg)
                self.log_msg(msg)
                msg = 'Epoch: {}\t ema validation loss: {:.5f}\t\n'.format(epoch+1, val_loss_ema)
                fp.write(msg)
            ...
            if best_mrd_loss_ema < self.best_val_loss_ema:
                self.best_val_loss_ema = best_mrd_loss_ema
                file_path = 'best_ema_model.ckpt-{:03d}.pt'.format(epoch + 1)
                trainer_flag = True if epoch > 20 else False
                self.save_checkpoint_ema(file_path, epoch, save_trainer=trainer_flag)
                self.log_msg("Find better validated model, saving to %s" % file_path)
    ...
    def _run_one_epoch(self, train=True, use_ema=False):
        if train:
        ...
        for i, batch in enumerate(pbar):
            ...
            if use_ema:
                pred_label = self.ema_model.forward_logit(sig.audio_data)
            else:
                pred_label = self.discriminator.forward_logit(sig.audio_data)
            
            if not train:
                pred_label = pred_label.unsqueeze(0)

            disc_loss_ce = self.ce_loss(pred_label, key)
            disc_loss = dag_fdd_loss(disc_loss_ce, 0.1)
            disc_losses = []
            ...
            if train:
                ...
                update_ema(self.discriminator, self.ema_model, decay=0.999)
            
            # 각 loss를 딕셔너리에 기록
            loss_values = {
                "disc": torch.mean(disc_loss_ce),
                "mrd_combine": disc_losses[0],
            }
            for k in loss_logs:
                loss_logs[k].append(loss_values[k].item())
            
            pbar.set_description("CE loss: {:.5f}".format(torch.mean(disc_loss_ce).item()))
        ...
        return avg_losses[0], avg_losses[1]
...
def main():
    ...
    #parser.add_argument('--checkpoint_path_student', default="/home/sam/singfake/outdir/mix_student/chkpt")
