...
from wav2vecAASIST import Wav2Vec2Model as model
...
class Train(object):
    def __init__(self, rank, a):
        ...
        self.model = model(a, self.device).to(self.device)
        self.rank = rank
        ...
        train_dataset = MFDataset(a.train_dir, train=True, chunk_size=64000, \
    ...
    def _reset(self):
        ...
        self.prev_val_loss, self.best_val_loss, self.best_eer_loss = float("inf"), float("inf"), float("inf")
        if os.path.exists(last_chkpt) and os.path.exists(last_chkpt.replace('model.ckpt', 'trainer.ckpt')):
            ...
            self.best_eer_loss = min(self.eer_loss[:self.start_epoch])
        else:
            self.start_epoch = 0
            self.global_step = 0
    ...
    def train(self):
        for epoch in range(self.start_epoch, self.epochs):
            ...
            with open(f'{self.log_dir}/val_curve.log', 'a') as fp:
                msg = 'Epoch: {}\t validation loss: {:.5f}, EER: {:.2f}\t\n'.format(epoch+1, val_loss, val_eer)
                fp.write(msg)
            ...
            if val_eer < self.best_eer_loss:
                self.best_eer_loss = val_eer
...
def main():
    ...
    parser.add_argument('--checkpoint_path', default="/ssd2/outputs/singfake/wav2vec_aasist")
