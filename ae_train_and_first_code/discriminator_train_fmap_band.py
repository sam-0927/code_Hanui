...
from model.discriminator_fmap import Discriminator_fmap, Discriminator_fmap_band
from utils import plot_melspectrogram, dag_fdd_loss
...
class Train(object):
    def __init__(self, rank, a):
        ...
        self.discriminator = Discriminator_fmap_band().to(self.device)
        ...
        self.scheduler_d = torch.optim.lr_scheduler.ExponentialLR(self.optim_d, 0.99)
    ...
    def _run_one_epoch(self, train=True):
        ...
        loss_logs = [] 
        loss_logs_list = [[] for _ in range(15)]
        avg_total = []
        for i, batch in enumerate(pbar):
            ...
            for i in range(3):
                for j in range(5):
                    # print(i, 5*j, 5*j+1, 5*j+2, 5*j+3, 5*j+4)
                    # print(5*i + j)
                    d_real_mrd_band = torch.cat([d_real_mrd[i][5*j], d_real_mrd[i][5*j+1], d_real_mrd[i][5*j+2], d_real_mrd[i][5*j+3], d_real_mrd[i][5*j+4]], dim=-1)
                    d_fake_mrd_band = torch.cat([d_fake_mrd[i][5*j], d_fake_mrd[i][5*j+1], d_fake_mrd[i][5*j+2], d_fake_mrd[i][5*j+3], d_fake_mrd[i][5*j+4]], dim=-1)
                    mrd_logit_list.append(self.discriminator.discriminators_mrd[5*i + j](d_real_mrd_band.detach(), d_fake_mrd_band.detach()))
            total_logit = sum(mrd_logit_list)
            total_loss = self.ce_loss(total_logit, key)
            '''
            disc_mrd_loss = []
            disc_total_loss = 0
            for i in range(len(mrd_logit_list)):
                loss = self.ce_loss(mrd_logit_list[i], key)
                disc_mrd_loss.append(loss)
                disc_total_loss += loss
                loss_logs_list[i].append(loss.item())
            '''
            if train:
            ...
            pbar.set_description("CE loss: {:.5f}".format(total_loss.item()))
...
def main():
    ...
    parser.add_argument('--checkpoint_path_disc', default='outdir/mix_draft/chkpt')
    parser.add_argument('--name_disc', default='best_model.ckpt-344_d.pt')

    #parser.add_argument('--checkpoint_path_student', default="outdir/mix_all/chkpt")
    #parser.add_argument('--name_student', default='model.ckpt-last_g.pt')
    #parser.add_argument('--checkpoint_path_disc', default='outdir/mix_all/chkpt')
    #parser.add_argument('--name_disc', default='model.ckpt-last_d.pt')

    # parser.add_argument('--checkpoint_path_student', default="outdir/mix_draft_spoof/chkpt")
    # parser.add_argument('--name_student', default='model.ckpt-last_g.pt')
    # parser.add_argument('--checkpoint_path_disc', default='outdir/mix_draft_spoof/chkpt')
    # parser.add_argument('--name_disc', default='model.ckpt-last_d.pt')
