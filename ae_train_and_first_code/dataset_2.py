...
class MFDataset(torch.utils.data.Dataset):
    ...
    def _get_tracklist(self):
        ...
        if self.train:
            ...
            if self.spoof:
                ...
                names = glob(os.path.join(path, '0_T01_*'))
        elif self.eval_t02:
            if self.spoof:
                names = glob(os.path.join(path, '1_T02_*'))
            else:
                names = glob(os.path.join(path, '0_T02_*'))
        elif self.eval_t04:
            if self.spoof:
                names = glob(os.path.join(path.replace('mixtures', 'mixtures_T04_only_per'), '1_T04_*'))
            else:
                names = glob(os.path.join(path.replace('mixtures', 'mixtures_T04_only_per'), '0_T04_*'))
        elif self.eval_t03:
            if self.spoof:
                names = glob(os.path.join(path.replace('mixtures', 'mixtures_T03'), 'aac_64k', '1_T03_*')) + \
                        glob(os.path.join(path.replace('mixtures', 'mixtures_T03'), 'mp3_128k', '1_T03_*')) 
            else:
                names = glob(os.path.join(path.replace('mixtures', 'mixtures_T03'), 'aac_64k', '0_T03_*')) + \
                        glob(os.path.join(path.replace('mixtures', 'mixtures_T03'), 'mp3_128k', '0_T03_*')) 
        return sorted(names)
    ...
    def _load_track(self, index):
        ...
        # if self.eval_t04:
        #     track_path_recon = Path(str(track_path).replace('/mixtures_T04_only_per/', '/student_make/recon_'))
        # else:
        #     track_path_recon = Path(str(track_path).replace('/mixtures/', '/student_make/recon_'))
        # track_path_recon = Path(str(track_path_recon).replace('.flac', '.wav'))
        
        sig = self._read_audio(track_path, frame_offset, num_frames)
        # sig_recon = self._read_audio(track_path_recon, frame_offset, num_frames)    # recon은 single channel
        if len(sig) > 1:
            if index % 2 == 0:
                sig = sig[0]
            else:
