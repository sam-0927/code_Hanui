...
class Train(object):
    def __init__(self, rank, a):
        ...
        self.student = Student_DAC().to(self.device)
        ...
        self.chkpt_path_teachder = os.path.join(a.checkpoint_path_teacher, a.name_teacher)
        checkpoint = load_checkpoint(self.chkpt_path_teachder, self.device)
        self.teacher.load_state_dict(checkpoint["generator"])
        print('Load checkpoint from '+self.chkpt_path_teachder)
        self.teacher.eval()
        ...
        self.student.load_state_dict(checkpoint["generator"])
    ...
    def _run_one_epoch(self, train=True):
        ...
        # loss_names = ['disc', 'mpd_1','mpd_2','mpd_3', 'msd','mrd_1','mrd_2','mrd_3','mrd_4','mrd_5']
        ...
        for i, batch in enumerate(pbar):
            ...
            sig = sig.unsqueeze(1)
            recons_t = self.teacher(sig, self.sampling_rate)
            recons_t = recons_t.detach()
            recons_t = AudioSignal(recons_t, 16000)
            recons_s,_,_ = self.student(sig, self.sampling_rate)
            recons_s = recons_s.detach()
            recons_s = AudioSignal(recons_s, 16000)
            
            sig = AudioSignal(sig, 16000)
            
            key = key.view(-1).type(torch.int64).to(self.device)
            if recons_s.audio_data.size() != recons_t.audio_data.size():
                breakpoint()

            pred_label = self.discriminator.forward_logit_combine(recons_t.audio_data, recons_s.audio_data)
            
            if not train:
                pred_label = pred_label.unsqueeze(0)
            loss = self.ce_loss(pred_label, key)
            disc_loss = loss
            disc_losses = []
            ...
            #     loss = self.ce_loss(pred_label[i], key)
            #     disc_loss += loss
            #     disc_losses.append(loss)
            ...
            if not train: 
                if not self.start_valid_spoof:
                    if key[0] == 1:
                        self.writer.add_audio('valid/teacher_spoof', recons_t[0].audio_data, self.global_step, 16000)
                        self.writer.add_figure('valid/teacher_spoof_mel', plot_melspectrogram(recons_t[0]), self.global_step)
                        self.writer.add_audio('valid/student_spoof', recons_s[0].audio_data, self.global_step, 16000)
                        self.writer.add_figure('valid/student_spoof_mel', plot_melspectrogram(recons_s[0]), self.global_step)
                        self.start_valid_spoof = True
                if not self.start_valid_bona:
                    if key[0] == 0:
                        self.writer.add_audio('valid/teacher_bonafide', recons_t[0].audio_data, self.global_step, 16000)
                        self.writer.add_figure('valid/teacher_bonafide_mel', plot_melspectrogram(recons_t[0]), self.global_step)
                        self.writer.add_audio('valid/student_bonafide', recons_s[0].audio_data, self.global_step, 16000)
                        self.writer.add_figure('valid/student_bonafide_mel', plot_melspectrogram(recons_s[0]), self.global_step)
        ...
        # avg_losses = tuple(np.mean(loss_logs[k]) for k in ["disc", 'mpd_1','mpd_2','mpd_3', 'msd','mrd_1','mrd_2','mrd_3','mrd_4','mrd_5'])
...
def main():
    ...
    parser.add_argument('--checkpoint_path_teacher', default="/home/sam/SingFake/models/dac/outdir/mix_draft/chkpt")
    parser.add_argument('--name_teacher', default='best_model.ckpt-344_g.pt')
    ...
    parser.add_argument("--training-epochs", metavar="NUM_EPOCHS", default=800,
