...
class Train(object):
    def __init__(self, rank, a):
        ...
        self.ocsoftmax_optim = torch.optim.SGD(self.ocsoftmax_loss.parameters(), lr=0.0003)
        self._reset()
    ...
    def save_checkpoint(self, checkpoint_path, epoch, save_trainer=True):
        ...
        torch.save({
                        ...
                        "ocsoftmax": self.ocsoftmax_loss.state_dict(),
                        "ocsoftmax_optim":self.ocsoftmax_optim.state_dict(),
                        }, d_chkpt_path)
        if logging:
    ...
    def _reset(self):
        ...
        if os.path.exists(last_chkpt_d):
            ...
            #self.ocsoftmax_optim.load_staet_dict(checkpoint["ocsoftmax_optim"])
    ...
    def _run_one_epoch(self, train=True):
        ...
        loss_logs = [] 
        loss_logs_b1, loss_logs_b2, loss_logs_b3 = [], [], []
        loss_logs_total = []
        for i, batch in enumerate(pbar):
            ...
            #disc_mrd_loss = self.ce_loss(summed_combine_logit_mrd, key)
            disc_mrd_loss, _ = self.ocsoftmax_loss(summed_combine_logit_mrd, key)

            disc_loss = disc_mrd_loss

            if train:
                self.optim_d.zero_grad()
                self.ocsoftmax_optim.zero_grad()
                disc_loss.backward()
                torch.nn.utils.clip_grad_norm_(self.discriminator.parameters(), 10.0)
                self.optim_d.step()
                self.ocsoftmax_optim.step()
                
            loss_logs.append(disc_mrd_loss.item())
            
            pbar.set_description("CE loss: {:.5f}".format(disc_mrd_loss.item()))
